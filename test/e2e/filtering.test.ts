...
describe("filtering", () => {
  ...
  describe("stream publisher and consumer", () => {
  ...
  describe("super stream publisher and consumer", () => {
    let superStreamName: string
    const routingKey = "0"

    beforeEach(async () => {
      superStreamName = createStreamName()
      await rabbit.createSuperStream(superStreamName)
    })
    ...
    it("can publish with filter value", async () => {
      const publisher = await client.declareSuperStreamPublisher(
        {
          ...
          routingStrategy: "key",
          filter: (msg) => msg.applicationProperties!["test"].toString(),
        },
        () => routingKey
      )
      const message1 = "test1"
      ...
      await eventually(async () => {
        expect((await rabbit.getQueueInfo(`${superStreamName}-0`)).messages).eql(3)
      }, 10000)
    }).timeout(10000)
      ...
      const publisher = await client.declareSuperStreamPublisher(
        {
          superStream: superStreamName,
          publisherRef: `my-publisher-${randomUUID()}`,
          filter: (msg) => (msg.applicationProperties ? msg.applicationProperties["test"].toString() : undefined),
        },
        () => routingKey
      )
      const applicationProperties1 = { test: "A" }
